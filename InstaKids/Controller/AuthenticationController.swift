...
//    func didFinishAuthenticating(isAuthenticated: Bool)
//}
...
class AuthenticationController: UIViewController {
    ...
    var didFinishAuthenticating: ((Bool) -> Void)?
    
    let passwordTextField: IKTextField = {
        let tf = IKTextField(padding: 24, height: 50)
    ...
    @objc fileprivate func handleLogin() {
        authenticationViewModel.performAuthentication { (err) in
            self.loginHUD.dismiss()
            
            self.dismiss(animated: true) {
                self.didFinishAuthenticating?(err == nil)
            }
        }
    }
    ...
    fileprivate let authenticationViewModel = AuthenticationViewModel()
